	...
	drawPlayerCards: function(wxUid, discard, gachaId, cardNum, cb) {
        ...
        Executor.redisGet(DBEnv_ZQ, discard_key, (e, r) => {
	        	if(e) {
	        		...
	        		let discardCards = [];
	        		if(r) {
	        			discardCards = JSON.parse(r);
	        		}
	        		for(let i = 0; i < discard.length; i++) {
	        			discardCards.push(discard[i]);
	        		}
	        			...
	        			if(e) {
	        				...
	        				if(cardIdList.length < cardNum) {
	        					for(let i = 0; i < discardCards.length; i++) {
	        						cardIdList.push(discardCards[i]);
	        					}
	        					discardCards = [];
	        				}
	        				let cardList = [];
	        				...
	        				if(cardList.length > 0) {
	        					...
	        					Executor.redisSet(DBEnv_ZQ, discard_key, JSON.stringify(discardCards), (e) => {
		        					if(e) {
		        						Log.error(`setPlayerDiscard redis error ${e}`);
		        						cb(new Error(GameCode.REDIS_ERROR), null);
		        					} else {
		        						Log.info(`Insert discard cards into redis ${JSON.stringify(discardCards)}`);
		        					}
		        				})
		        				//Store player's remained cards' id into card group
		        				Executor.redisSet(DBEnv_ZQ, cards_key, JSON.stringify(cardIdList), (e) => {
		        				...
		        				//Store $cardNum cards as play hand
		        				Executor.redisSet(DBEnv_ZQ, hand_key, JSON.stringify(handList), (e) => {
		        					if(e) {
		        						Log.error(`setPlayerHand redis error ${e}`);
		        						cb(new Error(GameCode.REDIS_ERROR), null);
